...
describe("avatar‑nft‑minter (SDK v2)", () => {
  ...
  const baseProgram = anchor.workspace
    .avatarNftMinter as Program<AvatarNftMinterIDL>;
  const idl = JSON.parse(JSON.stringify(baseProgram.idl)) as AvatarNftMinterIDL;

  const programCreator = new anchor.Program<AvatarNftMinterIDL>(
    idl,
    providerCreator
  );
  const programMinter = new anchor.Program<AvatarNftMinterIDL>(
    idl,
    providerMinter
  );
  ...
  describe("Avatar with fee", () => {
    ...
    const nftUri = `ipfs://${testIpfsHash}`;

    let firstMintPk: PublicKey;

    it("Fails mint if URI does not match avatar hash policy", async () => {
      try {
        await minterClient.mintNft({
          index: avatarIndex.toNumber(),
          name: "Bad URI",
          symbol: "BAD",
          uri: "ipfs://not-the-same-hash",
        });
        expect.fail("should throw");
      } catch (e: any) {
        expect(e.message).to.include("InvalidMetadataUri");
      }
    });
    ...
    it("Mints an NFT and pays fee", async () => {
      ...
      // mint/freeze authority must be revoked after mint
      const mintAcc = await getMint(connection, mintPk);
      expect(mintAcc.mintAuthority).to.equal(null);
      expect(mintAcc.freezeAuthority).to.equal(null);

      // extra mint must fail because mint authority is revoked
      let additionalMintFailed = false;
      try {
        await mintTo(
          connection,
          minterKeypair,
        ...
        additionalMintFailed = true;
      }
      expect(additionalMintFailed).to.be.true;
    });
    ...
    it("Mints a second NFT and accumulates fees", async () => {
      ...
      expect(dataAfter!.totalUnclaimedFees.eq(prevFees.add(mintingFeePerMint)))
        .to.be.true;
  ...
  describe("Avatar with zero fee", () => {
    ...
    const nftUri = `ipfs://${testIpfsHash2}`;
